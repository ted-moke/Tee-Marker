...
function tempTone (
  ...
  if (value < thresholds.tempBadLow || value > thresholds.tempBadHigh)
    return 'bad'
  if (value >= thresholds.tempGoodMin && value <= thresholds.tempGoodMax)
    return 'good'
  return 'neutral'
}
  ...
  return (
    <span className='inline-flex flex-wrap items-center gap-x-2 gap-y-1'>
      {weather.precipitationProbabilityPct !== null && (
        <span
          className={`inline-flex items-center gap-0.5 ${toneClass(
            rainTone(weather.precipitationProbabilityPct, thresholds)
          )}`}
        >
      ...
      {weather.temperatureF !== null && (
        <span
          className={`inline-flex items-center gap-0.5 ${toneClass(
            tempTone(weather.temperatureF, thresholds)
          )}`}
        >
      ...
      {weather.windSpeedMph !== null && (
        <span
          className={`inline-flex items-center gap-0.5 ${toneClass(
            windTone(weather.windSpeedMph, thresholds)
          )}`}
        >
